...
int32_t front_right = 0;
int32_t back_right = 0;
int32_t front_left = 0;
int32_t back_left=0;
...
PIDparameter FR_wheel={0,0,1.2,0.03};
PIDparameter FL_wheel={0,0,1.2,0.03};
PIDparameter BR_wheel={0,0,1.2,0.03};
PIDparameter BL_wheel={0,0,1.2,0.03};
...
void drive_kinematics(int RX_X2, int RX_Y1, int RX_X1,int S2)
{
    ...
    //Update using CAN bus chassis function (provided by Alex Wong)
		...
		if(S2 == 3){
			input_front_right = speedPID(1.5*front_right/8.0,CM1Encoder,&FR_wheel);
			input_back_right = speedPID(1.5*back_right/8.0,CM2Encoder,&BR_wheel);
			input_back_left = speedPID(1.5*back_left/8.0,CM4Encoder,&BL_wheel);
			input_front_left = speedPID(1.5*front_left/8.0,CM3Encoder,&FL_wheel);
		}
		if(S2 == 2){
			input_front_right = speedPID(2*front_right/8.0,CM1Encoder,&FR_wheel);
			input_back_right = speedPID(2*back_right/8.0,CM2Encoder,&BR_wheel);
			input_back_left = speedPID(2*back_left/8.0,CM4Encoder,&BL_wheel);
			input_front_left = speedPID(2*front_left/8.0,CM3Encoder,&FL_wheel);
		}
		Chassis_Set_Speed(input_front_right, input_back_right, input_front_left, input_back_left);
	
	
    //Chassis_Set_Speed(0.05*front_right, 0.05*back_right, 0.05*front_left, 0.05*back_left);
}
